...
const{reload}=useLoadingContext() as LoadingContextType
const createModalRef=useRef<any>(null)
const[selectedHabit,setSelectedHabit]=useState<SelectOptions|undefined>()

const parseHabitsToSelectOptions=():SelectOptions[]=>{
let options:SelectOptions[]=[{value:'all',text:'*All'},{value:'',text:'*No habit'}]
habits&&habits.forEach((habit:Habit)=>{options.push({value:habit.id,text:habit.title})})
return options}

useEffect(()=>{
;(()=>{
if(selectedHabit){
if(selectedHabit.value==='all'){setFilteredJournalEntries(journalEntries)}
...
setFilteredJournalEntries(filtered)}}
else{setFilteredJournalEntries(journalEntries)}})()},[habits,journalEntries,selectedHabit])
...
<Image src="/images/illustrations/undraw_personal_notebook_re_d7dc.svg"alt="Personal Journal"width={300}height={300}unoptimized/>
</div>
<div className="self-center text-right">Your Journal Logs</div>
<Select name="filterhabit"label="Filter by habit"options={parseHabitsToSelectOptions()}setSelectedValue={setSelectedHabit}required={false}/>
<StyledButton name="+ Add Log Entry"type={StyledButtonType.Primary}onClick={()=>createModalRef.current.open()}small/>
</div></div>
<JournalDashboard>
{!loading&&filteredJournalEntries&&filteredJournalEntries.map(journalEntry=>(
<JournalEntryCard key={journalEntry.id}journalEntry={journalEntry}/>))}
{loading&&<Loading/>}
</JournalDashboard></div></div>)}
